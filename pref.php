...
$PAGE->set_url('/local/expertrole/pref.php', array('id' => $userid, 'course' => $courseid));

list($user, $course) = useredit_setup_preference_page($userid, $courseid);

// Create form.
$courseform = new local_expertrole\expertrole_form(null, array('userid' => $user->id));
...
if ($courseform->is_cancelled()) {
    ...
    useredit_update_user_preference(['id' => $user->id,
        'preference_completeinterface' => $data->enablecompleteinterface]);
    local_expertrole\event\interface_updated::create_from_userid($user->id)->trigger();
    redirect($redirect);
}
